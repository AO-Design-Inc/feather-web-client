  ...
  switch (e.data.type) {
    ...
    case "offer":
      try {
        ...
        console.error(err);
        window.opener.postMessage(
          JSON.stringify({ type: "conerror", body: err })
        );
      }
      break;
    ...
    case "shortcut":
      datachannel.send(
        JSON.stringify({ type: action.SHORTCUT, body: e.data.body })
      );
      break;
...
window.onfocus = () => {
  ...
  window.opener.postMessage(
    JSON.stringify({ sender: self, server, type: "focus", program })
  );
};
...
window.addEventListener("beforeunload", () => {
  connection.close();
  window.opener.postMessage(
    JSON.stringify({ sender: self, type: "closed", program, server })
  );
});
